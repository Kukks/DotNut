    ...
    public class Wordlist
    {
        ...
        public static async Task<Wordlist> LoadWordList(string name)
        {
            ...
            if (result != null)
                lock (_LoadedLists)
                {
                    _LoadedLists.Remove(name);
                    _LoadedLists.Add(name, result);
                }
            return result;
        }
        ...
        {
            _words = words.Select(w => Mnemonic.NormalizeString(w)).ToArray();
            _Space = space;
            _Name = name;
        }
        ...
        public static Language AutoDetectLanguage(string[] words)
        {
            ...
            foreach (string s in words)
            {
                ...
                if (
                    Wordlist.ChineseTraditional.WordExists(s, out index)
                    && !Wordlist.ChineseSimplified.WordExists(s, out index)
                )
                {
        ...
        {
            return indices.Select(i => GetWordAtIndex(i)).ToArray();
        }
